...
export default function AdminContactPage() {
  ...
  const load = () =>
    fetchAdminJson<{ items: ContactItem[] }>("/api/contact").then((r) => {
      setItems(r.items);
    });
  ...
  const doDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetchAdmin(`/api/contact/${deleteId}`, { method: "DELETE" });
      await load();
      setDeleteId(null);
    } catch (e: unknown) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="font-heading text-4xl font-bold text-deep-black">Contact Submissions</h1>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-off-white border-b border-dark-grey/10">
            <tr>
              ...
              <th className="text-left p-4 font-medium text-deep-black">Subject</th>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          {viewItem && (
            <div className="grid gap-3 text-sm">
              ...
              <p><span className="font-medium text-dark-grey">Subject:</span> {viewItem.subject || "—"}</p>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this contact submission?</AlertDialogTitle>
