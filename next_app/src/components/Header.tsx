...
export default function Header() {
  ...
  const navLinks = [
    ...
    { name: 'Jobs & Career', path: '/applications' },
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                href={link.path}
                className={`font-paragraph text-base font-medium transition-colors relative ${isActive(link.path)
                  ? 'text-accent-red'
            ...
            <nav className="px-8 py-6 space-y-4">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`block font-paragraph text-lg font-medium py-2 transition-colors ${isActive(link.path)
                    ? 'text-accent-red'
                    : 'text-dark-grey hover:text-accent-red'
                    }`}
                >
